...
def save_checkpoint(records, start_idx, end_idx):
    """
    Saves a chunk of festival data to a checkpoint file
    Each file contains only the new data for festivals in the range
    """
    df = pd.DataFrame(records)
    checkpoint_file = f'weather_data_festivals_{start_idx+1}_to_{end_idx}.csv'
...
def combine_checkpoint_files(start_festival, end_festival):
    ...
    for file in sorted(checkpoint_files):
        ...
        df = pd.read_csv(file)
        # Filter for festivals within our range
    ...
    if all_data:
        ...
        # Get actual max festival ID from combined data
        actual_max_id = final_df['festival_id'].max()
        
        # Save combined file
        output_file = f'festivals_weather_{start_festival}_to_{actual_max_id}.csv'
        final_df.to_csv(output_file, index=False)
        print(f"\nCreated combined file: {output_file}")
        print(f"Total records: {len(final_df)}")
        
        # Return number of unique festivals as a check
...
if __name__ == "__main__":
    # Example usage:
    # For first chunk (festivals 1-130):
    main(start_festival=0, end_festival=50)
    
    # For second chunk (festivals 131-207), run:
    # main(start_festival=130, end_festival=207)
    
    # To resume from a specific festival:
    # main(start_festival=15, end_festival=130)  # Resume from festival 16